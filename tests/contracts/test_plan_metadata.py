...
_REQUIRED_PROOF_RE = re.compile(r'>\s+\*\*Required-Proof-Types:\*\*\s+`(?P<value>[^`]+)`')
...
def _extract_metadata(lines: list[str]) -> dict[str, object]:
    ...
    return {
        'lifecycle': lifecycle_match.group('value').strip() if lifecycle_match else None,
        'feature_class': feature_class_match.group('value').strip() if feature_class_match else None,
        'entry_point': entry_point_match.group('value').strip() if entry_point_match else None,
        'required_proof_types': required_proof_match.group('value').strip() if required_proof_match else None,
        'evidence_pack': evidence_pack_match.group('value').strip() if evidence_pack_match else None,
        'app_testable': app_testable_match.group('value').strip() if app_testable_match else None,
...
@pytest.fixture(scope='module')
def active_items(plan_items: dict[str, dict[str, object]]) -> dict[str, dict[str, object]]:
...
def test_retired_item_af_00_01_is_marked(plan_items: dict[str, dict[str, object]]) -> None:
...
def test_retired_items_excluded_from_active(active_items: dict[str, dict[str, object]]) -> None:
    '''AF-00-01 must not appear in the active items set.'''
    assert 'AF-00-01' not in active_items


def test_all_active_items_have_feature_class(active_items: dict[str, dict[str, object]]) -> None:
    ...
    for item_id, meta in active_items.items():
        ...
        if not fc:
            ...
            failures.append(f'{item_id}: invalid Feature-Class "{fc}" (must be one of {sorted(_VALID_FEATURE_CLASSES)})')
    assert not failures, 'Feature-Class issues:\n' + '\n'.join(failures)


def test_all_active_items_have_entry_point(active_items: dict[str, dict[str, object]]) -> None:
...
def test_all_active_items_have_acceptance_criteria(active_items: dict[str, dict[str, object]]) -> None:
...
def test_all_active_items_have_required_proof_types(active_items: dict[str, dict[str, object]]) -> None:
...
def test_all_active_items_have_evidence_pack_path(active_items: dict[str, dict[str, object]]) -> None:
    ...
    for item_id, meta in active_items.items():
        ...
        if not ep:
            ...
            if ep != expected:
                failures.append(f'{item_id}: Evidence-Pack is "{ep}", expected "{expected}"')
    assert not failures, 'Evidence-Pack issues:\n' + '\n'.join(failures)


def test_all_active_items_have_app_testable(active_items: dict[str, dict[str, object]]) -> None:
    ...
    for item_id, meta in active_items.items():
        ...
        if at is None:
            ...
            failures.append(f'{item_id}: App-Testable must be "true" or "false", got "{at}"')
