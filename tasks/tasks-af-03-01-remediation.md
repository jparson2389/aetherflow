...
> **Note:** Prerequisites 0.0/1.0 must complete before later tasks. Tasks 2.0-5.0 below describe the test-coverage work that has been implemented, but the validation gate (5.0) is not formally satisfied until reviewer sign-off (1.0) is recorded in `docs/evidence/AF-03-01.md` and `logs/verification/AF-03-01.json`.
...
- [ ] 2.0 Cover conservative fallback branch for unknown VID/PID devices (M-02)
  - [ ] 2.1 In `tests/integration/test_capture_opencv.py`, add `test_opencv_probe_conservative_fallback_for_unknown_device`: construct a `WindowsOpenCVCaptureProbe` with a device whose `device_id` has no matching VID/PID in `_DEVICE_CAPABILITY_RULES`; call `_fallback_modes_for(device)` and assert that only 30fps and 60fps modes at ≤1920×1080 are returned (no 120/144/240 entries)
  - [ ] 2.2 Assert in that test that the returned modes include at least `(1920, 1080, 30)` and `(1920, 1080, 60)`, and that no entry has `capture_fps in {120, 144, 240}`

- [ ] 3.0 Cover dropped-frame recording in session snapshot (M-02)
  - [ ] 3.1 In `tests/integration/test_capture_opencv.py`, add `test_opencv_capture_records_dropped_frames_in_session_metrics`: call `start_capture` for a supported mode, call `record_capture_sample(..., dropped=True)` a known number of times alongside normal frames, then call `stop_capture` and assert that `metrics.dropped_frames == expected_count`
  - [ ] 3.2 Confirm `metrics.frames_total == delivered + dropped` in the same test to validate the denominator used in `drop_rate`

- [ ] 4.0 Replace structural assertion in weak test (L-01)
  - [ ] 4.1 In `tests/integration/test_capture_opencv.py`, in `test_opencv_capture_enumerates_supported_modes_per_device`, remove `assert devices[0].device_id.startswith('USB\\')` (structural check on fake data)
  - [ ] 4.2 Replace with a behavioral assertion: assert that calling `plugin.supported_modes(devices[0].stable_id)` returns a non-empty list, and that all returned modes have `capture_width > 0` and `capture_fps > 0`

- [ ] 5.0 Verify remediation passes validation gate
  - [ ] 5.1 Run `uv run pytest tests/integration/test_capture_opencv.py tests/ui/test_capture_mode_matrix.py tests/integration/test_capture_stability.py -v` and confirm all tests pass with exit code 0
  - [ ] 5.2 Run `uv run python -m tools.check_quality` and confirm the full quality gate passes
