...
- `src/aetherflow/path/to/module.py` - Brief description of why this file is relevant.
- `tests/unit/test_module.py` - Unit tests for `module.py`.
- `src/aetherflow/path/to/another_module.py` - Brief description (e.g., runtime service or UI logic).
- `tests/integration/test_another_module.py` - Integration tests for `another_module.py`.
- `tools/helper_script.py` - Brief description (e.g., automation or developer tooling entrypoint).
- `tests/unit/test_helper_script.py` - Unit tests for `helper_script.py`.

### Notes

- Tests in this repo typically live under `tests/` and should reference the code they validate clearly.
- Use `uv run pytest [optional/path/to/test_file.py]` to run tests. Running without a path executes the full test suite discovered by pytest.
